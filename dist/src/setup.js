import { baseUrlTextInput, defineChannelSetupContract, defineTokenCredential, setSetupChannelEnabled, } from "openclaw/plugin-sdk/channel-setup";
import { hasConfiguredSecretInput, normalizeSecretInputString } from "openclaw/plugin-sdk/secret-input";
import { hasConfiguredFmsgApiKey, hasConfiguredFmsgApiUrl, hasConfiguredFmsgHomeChannel, normalizeFmsgAddress, normalizeFmsgApiUrl, rawFmsgConfig, } from "./config.js";
function patchFmsgConfig(cfg, patch, clearFields = []) {
...
function validateHomeChannel(value) {
    return normalizeFmsgAddress(value)
        ? undefined
        : "Enter an fmsg address in @user@domain form";
}
export const fmsgSetupContract = defineChannelSetupContract({
    fields: {
        apiUrl: {
            kind: "string",
...
function resolveSetupAccount(cfg) {
    return {
        configured: hasConfiguredFmsgApiKey(cfg) &&
            hasConfiguredFmsgApiUrl(cfg) &&
            hasConfiguredFmsgHomeChannel(cfg),
        config: rawFmsgConfig(cfg),
    };
}
export const fmsgSetupWizard = {
    channel: "fmsg",
    status: {
        configuredLabel: "fmsg configured",
        unconfiguredLabel: "fmsg needs an API URL, API key, and home address",
        resolveConfigured: ({ cfg }) => hasConfiguredFmsgApiKey(cfg) &&
            hasConfiguredFmsgApiUrl(cfg) &&
            hasConfiguredFmsgHomeChannel(cfg),
        resolveStatusLines: ({ cfg }) => {
            const lines = [];
            if (!hasConfiguredFmsgApiUrl(cfg))
                lines.push("fmsg API URL is required; no endpoint is assumed.");
            if (!hasConfiguredFmsgApiKey(cfg))
                lines.push("fmsg API key or SecretRef is required.");
    ...
    textInputs: [
        baseUrlTextInput({
            ...
            currentValue: (account) => account.config.apiUrl?.trim() || process.env.FMSG_API_URL?.trim() || undefined,
        ...
        }),
        {
            ...
            currentValue: ({ cfg }) => rawFmsgConfig(cfg).homeChannel?.trim() || process.env.FMSG_HOME_CHANNEL?.trim() || undefined,
            validate: ({ value }) => validateHomeChannel(value),
            normalizeValue: ({ value }) => normalizeFmsgAddress(value) ?? value.trim(),
            applySet: ({ cfg, value }) => patchFmsgConfig(cfg, { homeChannel: normalizeFmsgAddress(value) ?? value.trim() }),
        },
    ],
    credentials: [
        defineTokenCredential({
            ...
            patchAccount: ({ cfg, mode, patch, clearFields }) => patchFmsgConfig(cfg, patch, mode === "env" ? ["apiKey"] : clearFields),
    ...
    finalize: async ({ cfg, prompter }) => {
        ...
        if (owners.some((entry) => String(entry).toLowerCase() === ownerEntry))
            return;
