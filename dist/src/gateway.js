...
export async function startFmsgGatewayAccount(ctx) {
    ...
    if (!channelRuntime?.inbound?.dispatch)
        throw new Error("fmsg requires the OpenClaw channel runtime");
    ...
    try {
        await runFmsgConnection({
            ...
            ...(log ? { log } : {}),
            onReconnectAttempt: (reconnectAttempts) => ctx.setStatus({ accountId: account.accountId, reconnectAttempts }),
