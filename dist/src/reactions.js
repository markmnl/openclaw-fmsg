...
import { jsonResult, readReactionParams, resolveReactionMessageId, } from "openclaw/plugin-sdk/channel-actions";
...
function resolveConfiguredAccount(cfg, accountId) {
    try {
...
export const fmsgMessageActions = {
    ...
    handleAction: async ({ action, params, cfg, accountId, toolContext }) => {
        ...
        if (message.terminal)
            throw new Error(`fmsg message ${messageId} is terminal and cannot be reacted to`);
...
function resolveSessionPeer(service, rootId, ownAddress) {
    const root = service.state.getMessage(rootId);
    if (!root)
        return undefined;
    const sender = normalizeFmsgAddress(root.from);
    if (sender && sender !== ownAddress)
        return sender;
    const others = [...new Set([root.from, ...root.to, ...root.addTo]
            .map(normalizeFmsgAddress)
            .filter((address) => Boolean(address) && address !== ownAddress))];
    return others.length === 1 ? others[0] : undefined;
}
function formatReactionEvent(change, messageId) {
...
export async function handleFmsgReaction(params) {
    ...
    if (service.state.getMessage(message.id))
        service.state.assignMessage(message);
    const diff = await service.state.recordReactionSnapshot(message, {
        seedIfMissing: params.source === "catch-up",
    });
    if (diff.changes.length === 0 || service.config.reactionNotifications === "off")
        return;
    const ownAddress = (await service.client.getToken()).sender;
    ...
    for (const change of diff.changes) {
        if (change.from === ownAddress || !isFmsgSenderAllowed(service.config, change.from))
            continue;
